...
class ESIG_SAD {
    ...
    public function display_document($atts) {
       ...
       $esigLocale = esigget("_locale",$_REQUEST);
        ...
        $html = '
			<p>
				<label class="esig-visuallyhidden">' . __('Your email address', 'esig') . '</label><input required type="email" class="form-control" placeholder="' . __('Your email address', 'esig') . '" id="esig-sad-email" name="esig-sad-email" value="' . $legal_email . '"/>
			</p>
        ';
        ...
        if (!isset($_POST['recipient_signature']) && empty($_POST['recipient_signature']) && !isset($_POST['esignature_in_text']) && empty($_POST['esignature_in_text'])) {
            ...
            // If type siganture is not valid 
            if(!empty($esign_text) && !$api->validation->esig_valid_string($esign_text))
            {
                $api->notice->set('e-sign-red-alert email', 'Signature is not valid');
                wp_redirect(esc_url($_SERVER['HTTP_HOST'] . $_SERVER["REQUEST_URI"]));
                exit;
            ...
            $template_data = array(
                ...
                "recipient_signature" => esigpost('recipient_signature'),
    ...
    private static function single_activate() {
        ...
        if (get_option('WP_ESignature__Stand_Alone_Documents_documentation')) {
            update_option('WP_ESignature__Stand_Alone_Documents_documentation', 'https://wpe.approveme.com/article/stand-alone-documents-add-on/');
        } else {

            add_option('WP_ESignature__Stand_Alone_Documents_documentation', 'https://wpe.approveme.com/article/stand-alone-documents-add-on/');
        }
    }
