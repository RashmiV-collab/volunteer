 ...
 */
class WP_Event_Manager_WCPL_Subscriptions {
	...
	public function __construct() {
		if ( class_exists( 'WC_Subscriptions_Synchroniser' ) && method_exists( 'WC_Subscriptions_Synchroniser', 'save_subscription_meta' ) ) {
			add_action( 'woocommerce_process_product_meta_event_package_subscription', 'WC_Subscriptions_Synchroniser::save_subscription_meta', 10 );
		}
	...
	public function updated_post_meta( $meta_id, $object_id, $meta_key, $meta_value ) {
		if ( 'event_listing' === get_post_type( $object_id ) && '' !== $meta_value && '_event_expires' === $meta_key ) {
			$_package_id = get_post_meta( $object_id, '_package_id', true );
			$package     = wc_get_product( $_package_id );

			if ( ( $package instanceof WC_Product_Event_Package_Subscription
			       || $package instanceof WC_Product_Resume_Package_Subscription )
			     && 'listing' === $package->get_package_subscription_type()
			) {
				update_post_meta( $object_id, '_event_expires', '' ); // Never expire automatically
			}
		}
	}
	...
	public function woocommerce_is_subscription( $is_subscription, $product_id ) {
		...
		if ( $product && $product->is_type( array( 'event_package_subscription') ) ) {
		...
		if ( 'event_listing' === $post->post_type ) {
			...
			if ( $package_product_id ) {
				...
				if ( 'listing' === $subscription_type ) {
					$new_count = $wpdb->get_var( $wpdb->prepare( "SELECT package_count FROM {$wpdb->prefix}emwcpl_user_packages WHERE id = %d;", $package_id ) );
					$new_count --;
		...
		if ( $id > 0 ) {
			...
			if ( 'event_listing' === $post_type ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$package  = $wpdb->get_row( $wpdb->prepare( "SELECT package_count, package_limit FROM {$wpdb->prefix}emwcpl_user_packages WHERE id = %d;", $package_id ) );
	...
	/**
	 * Subscription has expired - cancel event packs
	 *
	 * @param WC_Subscription $subscription
	 * @param bool            $paused
	 */
	public function subscription_ended( $subscription, $paused = false ) {
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			$parent_id         = ! empty( $parent ) ? wpem_paid_listings_get_order_id( $parent ) : null;
			$legacy_id         = isset( $parent_id ) ? $parent_id : wpem_paid_listings_get_order_id( $subscription );
			$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}emwcpl_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", wpem_paid_listings_get_order_id( $subscription ), $legacy_id, $item['product_id'] ) );
			...
			if ( $user_package ) {
				...
				if ( 'listing' === $subscription_type ) {
					...
					foreach ( $listing_ids as $listing_id ) {
						...
						update_post_meta( $listing_id, '_expired_subscription_id', wpem_paid_listings_get_order_id( $subscription ) );
					}
				}
			}
		}// End foreach().

		delete_post_meta( wpem_paid_listings_get_order_id( $subscription ), 'wpem_paid_listings_subscription_packages_processed' );
	}
	...
	public function subscription_activated( $subscription ) {
		...
		if ( get_post_meta( wpem_paid_listings_get_order_id( $subscription ), 'wpem_paid_listings_subscription_packages_processed', true ) ) {
			return;
		}
		...
		$wpdb->delete( "{$wpdb->prefix}emwcpl_user_packages", array(
			'order_id' => wpem_paid_listings_get_order_id( $subscription ),
		) );
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'event_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {

				// Give packages to user
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = wpem_paid_listings_give_user_package( $subscription->get_user_id(), $product->get_id(), wpem_paid_listings_get_order_id( $subscription ) );
				}
				 ...
				 */
				if ( 'listing' === $subscription_type ) {
					$listing_ids = (array) $wpdb->get_col( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key=%s AND meta_value=%s", '_expired_subscription_id', wpem_paid_listings_get_order_id( $subscription ) ) );
				} else {
					$listing_ids = array();
				}
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
						wpem_paid_listings_approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
		...
		update_post_meta( wpem_paid_listings_get_order_id( $subscription ), 'wpem_paid_listings_subscription_packages_processed', true );
	}

	/**
	 * Subscription renewed - renew the event pack
	 *
	 * @param WC_Subscription $subscription
	 */
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( 'package' === $subscription_type ) {
				if ( ! $wpdb->update(
					...
					wpem_paid_listings_give_user_package( $subscription->get_user_id(), $item['product_id'], wpem_paid_listings_get_order_id( $subscription ) );
				}
			} else {
				// Otherwise the listings stay active, but we can ensure they are synced in terms of featured status etc
				if ( $user_package_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}emwcpl_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", wpem_paid_listings_get_order_id( $subscription ), $legacy_id, $item['product_id'] ) ) ) {
					foreach ( $user_package_ids as $user_package_id ) {
						$package = wpem_paid_listings_get_user_package( $user_package_id );

						if ( $listing_ids = wpem_paid_listings_get_listings_for_package( $user_package_id ) ) {
							foreach ( $listing_ids as $listing_id ) {
								// Featured or not
	...
	public function subscription_item_switched( $order, $subscription, $new_order_item_id, $old_order_item_id ) {
		...
		$new_subscription = (object) array(
			'id'           => wpem_paid_listings_get_order_id( $subscription ),
	...
	public function switch_package( $user_id, $new_subscription, $old_subscription ) {
		...
		$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}emwcpl_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $old_subscription->id, $legacy_id, $old_subscription->product_id ) );

		if ( $user_package ) {
			// If invalid, abort
			if ( ! $new_subscription->product->is_type( array( 'event_package_subscription' ) ) ) {
				return false;
			}

			// Give new package to user
			$switching_to_package_id = wpem_paid_listings_give_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
			...
			if ( $new_subscription->product instanceof WC_Product_Event_Package_Subscription ) {
				$does_new_subscription_feature_listings = $new_subscription->product->is_event_listing_featured();
			}
				...
				foreach ( $listing_ids as $listing_id ) {
					...
					if ( ! $is_upgrade ) {
						...
						if ( apply_filters( 'event_manager_event_listing_affects_package_count', true, $listing_id ) ) {
							wpem_paid_listings_increase_package_count( $user_id, $switching_to_package_id );
						}
					...
					// Fire action
					do_action( 'wpem_paid_listings_switched_subscription', $listing_id, $user_package );
				}
			}
		}// End if().
	}
}
WP_Event_Manager_WCPL_Subscriptions::get_instance();
