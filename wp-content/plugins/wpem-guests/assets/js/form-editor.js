jQuery(document).ready(function($) {
	...
	$('.wp-event-manager-guest-lists-form-editor')
		.on( 'init', function() {
			$(this).sortable({
				items:'tr',
				cursor:'move',
		...
		.on( 'click', '.delete-field', function() {
			if ( window.confirm( wp_event_manager_guests_form_editor.cofirm_delete_i18n ) ) {
				$(this).closest('tr').remove();
			}
		})
		.on( 'click', '.reset', function() {
			if ( window.confirm( wp_event_manager_guests_form_editor.cofirm_reset_i18n ) ) {
				return true;
			}
			return false;
		})
		.on( 'click', '.add-field', function() {
			...
			$tbody.append( row );
			$('.wp-event-manager-guest-lists-form-editor').trigger( 'init' );
			return false;
		});
