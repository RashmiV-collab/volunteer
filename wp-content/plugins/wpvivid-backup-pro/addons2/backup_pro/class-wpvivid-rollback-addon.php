...
class WPvivid_Rollback_Addon
{
    ...
    public function rollback_upload_event()
    {
        ...
        $rollback_delete_local=get_option("wpvivid_rollback_delete_local",0);
        //
    ...
    public function export_dashboard_info($json)
    {
        ...
        $json['data']['wpvivid_rollback_delete_local']        = $rollback_delete_local;
    ...
    public function backup($response, $args)
    {
        ...
        if(!empty($plugin))
        {
            ...
            if($this->get_enable_auto_backup_status($slug))
            {
                ...
                if (!$ret)
                {
                    do_action('wpvivid_do_rollback_mail_report', 'error', 'plugin', $slug, $path);
                    $last_error = $pclzip->errorInfo(true);
                ...
                else
                {
                    ...
                    do_action('wpvivid_do_rollback_mail_report', 'Succeeded', 'plugin', $slug, $path);
                }
            }
        }
        ...
        if(!empty($theme))
        {
            if($this->get_theme_enable_auto_backup_status($theme))
            {
                ...
                if (!$ret)
                {
                    do_action('wpvivid_do_rollback_mail_report', 'error', 'theme', $wp_theme, $path);
                    $last_error = $pclzip->errorInfo(true);
                    return new WP_Error('rollback_backup_failed',$last_error);
                }
                else
                {
                    ...
                    do_action('wpvivid_do_rollback_mail_report', 'Succeeded', 'theme', $theme, $path);
                }
            }
        }
    ...
    public function output_settings()
    {
        ...
        if($rollback_delete_local)
        {
            $rollback_delete_local = 'checked';
        }
        else
        {
            $rollback_delete_local = '';
        }
        ?>
        <div>
        ...
        <div style="margin-bottom: 10px;">
            ...
            <p>
                <label>
                    <input type="checkbox" class="wpvivid-rollback-remote" id="wpvivid_rollback_delete_local" <?php echo $rollback_delete_local;?>> Delete versioning backups from localhost after they are uploaded to cloud storage.
                </label>
            </p>
            <p>
                <label>
        ...
        <script>
            ...
            function wpvivid_submit_rollback_setting()
            {
                ...
                var rollback_delete_local = '0';
                if(jQuery('#wpvivid_rollback_delete_local').prop('checked'))
                {
                    rollback_delete_local = '1';
                }
                else
                {
                    rollback_delete_local = '0';
                }
                ...
                var ajax_data = {
                    ...
                    'rollback_delete_local':rollback_delete_local,
    ...
    public function set_rollback_setting()
    {
        ...
        if (isset($_POST['rollback_delete_local']))
        {
            $rollback_delete_local = sanitize_key($_POST['rollback_delete_local']);
            if($rollback_delete_local=="1")
            {
                $rollback_delete_local=1;
            }
            else
        ...
        update_option('wpvivid_rollback_delete_local', $rollback_delete_local);
