				...
				while ($alerts->have_posts()):
					...
					<tr>
						...
						<td data-title="<?php _e('Keywords', 'wp-event-manager-alerts'); ?>" class="alert_keyword"><?php
							if(get_post_meta(get_the_ID(), '_alert_keyword', true))
								echo esc_html(get_post_meta(get_the_ID(), '_alert_keyword', true));
							else if(get_post_meta(get_the_ID(), 'alert_keyword', true))
								echo esc_html(get_post_meta(get_the_ID(), 'alert_keyword', true));
							else
								echo '&ndash;';?>
						</td>
						<?php if(get_option('event_manager_enable_categories') && wp_count_terms('event_listing_category') > 0) : ?>
						<td data-title="<?php _e('Categories', 'wp-event-manager-alerts'); ?>" class="alert_category"><?php
							$terms = wp_get_post_terms(get_the_ID(), 'event_listing_category', array('fields' => 'names'));
							echo $terms ? esc_html(implode(', ', $terms)) : '&ndash;';?>
						</td>
						<?php endif; ?>
						<?php if(taxonomy_exists('event_listing_tag')) : ?>
						<td data-title="<?php _e('Tags', 'wp-event-manager-alerts'); ?>" class="alert_tag"><?php
							$terms = wp_get_post_terms(get_the_ID(), 'event_listing_tag', array('fields' => 'names'));
							echo $terms ? esc_html(implode(', ', $terms)) : '&ndash;';?>
						</td>
						<?php endif; ?>					
						<td  data-title="<?php _e('Location', 'wp-event-manager-alerts'); ?>" class="alert_location"><?php
							...
							else if(get_post_meta(get_the_ID(), 'alert_location', true))
								echo esc_html(get_post_meta(get_the_ID(), 'alert_location', true));
							else
								echo '&ndash;';?>
						</td>
						<td data-title="<?php _e('Frequency', 'wp-event-manager-alerts'); ?>" class="alert_frequency"><?php
							if(get_post_meta(get_the_ID(), '_alert_frequency', true))
								$freq = get_post_meta(get_the_ID(), '_alert_frequency', true);
							else if(get_post_meta(get_the_ID(), 'alert_frequency', true))
								$freq = get_post_meta(get_the_ID(), '_alert_frequency', true);
