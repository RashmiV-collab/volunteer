...
class WP_E_aboutsController extends WP_E_appController {
    ...
    public function systeminfo($data_return = false) {
        ...
        $this->view->render('systeminfo','about', $template_data);
    }
        ...
        if (WP_E_Sig()->document->document_document_page_exists($pageID)) {
            ...
            if ($page_data) :
                if (function_exists('has_shortcode')) {
                    ...
                    $core_html .= $page_data->post_title . " \t\t " . $permalink . "No shortcode found\n\t\t\t";
                }
            endif;
        }
        global $wpdb;
        ...
        if($system_index){
            ...
            if(array_key_exists($system_index, $array)){
                if($array[$system_index] == "memory_limit"){ 
                    $result['content'] = __("Checking memory limit at least 96M", "esig");
                    
                   
                }
                if($array[$system_index] == "php"){
                    $result['content'] = __("Checking for php version...", "esig");
                   
                   
                }
                if($array[$system_index] == "wordpress"){
                    $result['content'] = __("Checking for Wordpress version...", "esig");
                    
                   
                }  
                if($array[$system_index] == "curl"){ 
                    $result['content'] = __("Checking for PHP Curl...", "esig");
                    
                }
                if($array[$system_index] == "mcrypt"){ 
                    $result['content'] = __("Checking for PHP Openssl...", "esig");
                    
                }
                if($array[$system_index] == "remote"){
                    $result['content'] = __("Checking for REMOTE GET/POST...", "esig");
                   
                }
                if($array[$system_index] == "gd"){
                    $result['content'] = __("Checking for Php GD library...", "esig");
