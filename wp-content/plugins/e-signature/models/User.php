...
class WP_E_User extends WP_E_Model {
    ...
    public function get_esig_signer_name($user_id, $document_id) {
        ...
        } elseif ($signers = $this->signer->get_document_signer_info($user_id, $document_id)) {
    ...
    public function esig_get_administrator_displayname() {
        ...
        $administratorDisplayName = stripslashes_deep(esigget("display_name",$user_details));
    ...
    public function esig_get_administrator_email() {
        ...
        $admin_user_id = $this->esig_get_super_admin_id();
        $user_details = get_userdata($admin_user_id);
        return esigget("user_email",$user_details);
    }
    ...
    {
        $wp_user_id = $this->getCurrentWPUserID();
        $isAdmin = $this->wpdb->get_var($this->wpdb->prepare("SELECT is_admin FROM " . $this->table . " WHERE wp_user_id=%d", $wp_user_id));
        return wp_validate_boolean($isAdmin);
    }
