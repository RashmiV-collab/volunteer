...
class WP_E_Document extends WP_E_Model {
    ...
    public function getDocumentByID($id) {
        ...
        $invite_get =  $this->validation->esig_clean(esigget('invite'));

        if ($invite_get != NULL || $invite_get != "") {

            // invited checksum verify
            $checksum = esigget('csum');
        ...
        if (esigget("document_content",$document) == NULL || empty(esigget("document_content", $document))) {
    ...
    public function create_default_document_page($page_id) {
        ...
        // if trash page not exits then trying to create the new page
            ...
            $doc_id = wp_insert_post($doc_page, true);
    ...
    public function fetchAllOnStatus($status, $super_admin_result = false, $pagenum = 1, $limit = false) {
        ...
        $orderBy  = (esigget("orderby")) ? esigget("orderby"): "document_id";
        ...
        if ($status == 'all') {
            ...
            return $this->wpdb->get_results(
                            $this->wpdb->prepare(
                                    "SELECT * FROM " . $this->table . " WHERE document_status=%s ORDER BY %s DESC LIMIT %d,%d", $status,$orderBy, $offset, $limit
                            )
            );
        } else {
            ...
            if ($admin_user_id == $wp_user_id) {
                return $this->wpdb->get_results(
                                $this->wpdb->prepare(
                                        "SELECT * FROM " . $this->table . " WHERE document_status=%s ORDER BY ". $orderBy ." DESC LIMIT %d,%d", $status, $offset, $limit
                                )
                );
            } else {
                ...
                if (class_exists('ESIG_USR_ADMIN') && $status == "esig_template") {
                    ...
                    $docs = $this->wpdb->get_results(
                            $this->wpdb->prepare(
                                    "SELECT * FROM " . $this->table ." WHERE document_status=%s ORDER BY " . $orderBy . " DESC", $status)
                    );
                ...
                return $this->wpdb->get_results(
                                $this->wpdb->prepare(
                                        "SELECT * FROM " . $this->table ." WHERE user_id=%d and document_status=%s ORDER BY " . $orderBy . " DESC LIMIT %d,%d", $wp_user_id, $status, $offset, $limit
                                )
                );
            }
        }
...
EOL;
    ...
    public function esig_date($document_id = false) {
        ...
        if (!empty($doc_timezone)) {
            // date_default_timezone_set($doc_timezone);
            $date = new DateTime("now", new DateTimeZone($doc_timezone));
